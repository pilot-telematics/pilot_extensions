...
В результате дай полный код файлов и пошаговую инструкцию куда их положить и какой Module.js URL зарегистрировать в PILOT.
```
...
2. Дерево файлов.
3. Полный код каждого файла.
4. `Module.js` как единственную runtime-точку входа.
5. `doc/index.html` без `<script>`.
